...
public class MainActivity extends AppCompatActivity {
    ...
    void showList(){
        // Return phone to portrait when leaving the player; Android TV stays unchanged.
        if(!isTvDevice()) setRequestedOrientation(android.content.pm.ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED);
    ...
    void play(int index){if(index<0||index>=shown.size())return;current=index;
        // On phones, opening a channel switches to landscape fullscreen.
        if(!isTvDevice()) setRequestedOrientation(android.content.pm.ActivityInfo.SCREEN_ORIENTATION_SENSOR_LANDSCAPE);Channel c=shown.get(index);if(player!=null)player.release();bandwidthMeter=new DefaultBandwidthMeter.Builder(this).build();player=new ExoPlayer.Builder(this).setBandwidthMeter(bandwidthMeter).build();
